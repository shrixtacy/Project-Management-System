...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // req.url is the original full path e.g. /api/auth/create-user
  const url = req.url || '';
  const method = req.method || 'GET';

  // ── GET /api/health ────────────────────────────────────────────────────────
  if (method === 'GET' && url === '/api/health') {
    return res.json({ status: 'ok', env: { hasSupabase: !!process.env.SUPABASE_URL, hasCloudinary: !!process.env.CLOUDINARY_CLOUD_NAME } });
  }

  // ── POST /api/auth/create-user ─────────────────────────────────────────────
  if (method === 'POST' && url.startsWith('/api/auth/create-user')) {
  ...
  if (method === 'GET' && url.startsWith('/api/upload/signature')) {
    try {
      ...
      const signature = cloudinary.utils.api_sign_request({ timestamp, folder: 'pms_deliverables' }, apiSecret);
  ...
  if (method === 'GET' && url.startsWith('/api/upload/proxy-download')) {
    try {
  ...
  if (method === 'POST' && url.startsWith('/api/upload/delete-files')) {
    try {
      const { urls } = req.body;
